...
export async function postOneProduct(req, res, next) {
    try {
        ...
        if (!name || !description || !price || !category) {
            const error = new Error(`Las propiedades ${name}, ${description}, ${price} y ${category} son obligatorias`);
            error.status = 400;
            return next(error);
        }
    
        // Crear el nuevo producto
        const newProducto = new Producto({ name, description, price, category, photo: photo || '' });
